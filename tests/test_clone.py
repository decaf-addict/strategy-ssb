...
def test_clone(accounts, Strategy, strategy, strategist, rewards, keeper, token2, user, vault, vault2, amount2,
               balancer_vault,
               pool, chain, gov, balancer_minter,
    ...
    with brownie.reverts("Strategy already initialized"):
        strategy.initialize(vault, strategist, rewards, keeper, balancer_vault, pool, gauge_factory, balancer_minter, 10, 10, 100_000, 2 * 60 * 60)

    transaction = strategyFactory.clone(vault2, strategist, rewards, keeper, pool)
    cloned_strategy = Strategy.at(transaction.return_value)

    with brownie.reverts("Strategy already initialized"):
        cloned_strategy.initialize(vault, strategist, rewards, keeper, balancer_vault, pool, gauge_factory, balancer_minter, 10, 10, 100_000,
                                   2 * 60 * 60, {'from': gov})
